...
def get_pdfs(dataset, *args):
    ...
    dataset : pandas.DataFrame
        Dataframe must include all variable in *args.
    *args : list
...
def shuffleData(a_dataframe, *args):
    """
    Shuffle every column of a dataframe. 
...
def get2DMI(dt, var1, var2, norm = False):
    """
    ...
    ----------
    dt : pandas.DataFrame
        The dataset that the algorithm will run on.
    var1 : str
...
def getPIDs(pid, source1, isotope, target, ifNorm = False):
    '''
    ...
    ## create dataframe with columns as [source1, isotope, target]
...
def getinterations(iters):
    start = time()
    OverallDf = pd.DataFrame(np.full((47, 55), np.nan))
    # NeonPath = "../Processed data 24hiso/"            ## this is the folder that stores alltime datasets
    # NeonPath = "../alltime_data/"                     ## this folder stores alltime datasets processed in 2023
    NeonPath = "../alltime_time_correct_data/"          ## this folder stores alltime datasets processed in 2023
                                                        ## with the LOCAL time corrected to UTC
    ...
    for s in np.arange(len(cleanUpData)):
        ...
        dfs['dExcess'] = dfs['d2H'] - 8*dfs['d18O'] ##define d-Excess
        data = dfs.drop(columns=['d18O']).copy()

        cols = list(data.columns)[:6]
        C13Iso = data[cols + ['d13C']].dropna().copy()
        C13Iso = shuffleData(C13Iso, 'NEE_U50_f', 'LE_U50_f', 'VPD_f', 'Tair_f', 'wspd',
                           'Rg_f', 'd13C')
        
        
        dExIso = data[cols + ['dExcess']].dropna().copy()
        dExIso = shuffleData(dExIso,  'NEE_U50_f', 'LE_U50_f', 'VPD_f', 'Tair_f', 'wspd',
                           'Rg_f','dExcess')
        
        H2Iso = data[cols + ['d2H']].dropna().copy()
        H2Iso = shuffleData(H2Iso, 'NEE_U50_f', 'LE_U50_f', 'VPD_f', 'Tair_f', 'wspd',
                           'Rg_f','d2H')
         ###data indicator
        ...
        #############below the code are very redundant so we need to simplify it 
        if nptsC13 >= 100:#
        ...
        if nptsH2 >= 100:
            ...
            for ll in np.arange(500):
                ...
                sampleH2 = H2Iso.sample(n = 100, replace = False)
        ...
        if nptsdEx >= 100:
            ...
            for kk in np.arange(500):
                ...
                sampledEx = dExIso.sample(n = 100, replace = False)
    ...
    OverallDf.to_csv(f'../ShuffleMIs/alltime_2023_time_corrected/Iteration{iters}_alltime_time_corrected.csv',  ## changed to alltime folder 
                     index = False)                                               ## changed to alltime time corrected folder
    end = time()
    print(f"finishing {iters} iteration")
    print()
    return int(end - start)

def mainFunction():
    numIters = np.arange(0,50)  
    pools = ProcessPoolExecutor(max_workers = 10)
    results = list(pools.map(getinterations, numIters))
